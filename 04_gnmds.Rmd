...
env.var_sub5 <- subset(env.var_2, species_richness >5)
...
#  #  1   2   3   5   6 
#  # 27   0  13   2 177 
# env.var_sub5_grain <- env.var_sub5
# env.var_sub5_grain$grain_size_stand_f <- as.numeric(env.var_sub5_grain$grain_size_stand_f)
# env.var_sub5_grain$grain_size_stand_f <- ifelse(env.var_sub5_grain$grain_size_stand_f >= 4, 6, env.var_sub5_grain$grain_size_stand_f)
# table(env.var_sub5_grain$grain_size_stand_f)
#  #  1   3   6 
#  # 27  13 179
# env.var_sub5_grain$grain_size_stand_f <- as.factor(env.var_sub5_grain$grain_size_stand_f)
...
#      ylim =  c(-3, 3), xlim = c(-3, 3))
  ...
  points(gnmds1[env.var_sub5$method == "nat"], gnmds2[env.var_sub5$method == "nat"], pch = 3, col = "cyan3", cex = 0.7)
  points(gnmds1[env.var_sub5$method == "ref"], gnmds2[env.var_sub5$method == "ref"], pch = 2, col = "darkorange", cex = 0.7)
  points(gnmds1[env.var_sub5$method == "seed"], gnmds2[env.var_sub5$method == "seed"], pch = 1, col = "gold2", cex = 0.7)
  points(gnmds1[env.var_sub5$method == "pn"], gnmds2[env.var_sub5$method == "pn"], pch = 4, col = "magenta", cex = 0.7)

# Legger på vektorer for environmental variables
  mds.z <- envfit(scores(gnmds.full, choices = 1:2, origin = TRUE), env.var_sub5, 999)
...
```{r}
gnmds.full <- mds.best
save(gnmds.full, file = "gnmds_full_e09_2020_08_31.rda")

save(mds.z, file="env.var_fit_e095_2020_09_07.rda") # this was with changed grain size categories
...
# # env.var_full_sub5 <- env.var_full_sub5[, -c(41)]
...
uid <- unique(env.var_full_sub5$id)

# Akse1
mean_gnmds1 <- rep(0, times=length(uid))
for(i in 1:length(uid)) {
  mean_gnmds1[i] <- mean(gnmds1[env.var_full_sub5$id == (uid[i])], na.rm=T)
}
mean_gnmds1
...
for(i in 1:length(uid)) {
  mean_gnmds2[i] <- mean(gnmds2[env.var_full_sub5$id == (uid[i])], na.rm=T)
}
mean_gnmds2
...
mean_gnmds_data <- cbind(mean_gnmds1_data, mean_gnmds2)
env.var_full_sub5 <- cbind(env.var_full_sub5, mean_gnmds_data)
names(env.var_full_sub5)
env.var_full_sub5 <- env.var_full_sub5[, -c(46)]

str(env.var_full_sub5)

save(env.var_full_sub5, file = "envvar_full_sub5_2020_08_31.rda") # columns 43 and 44 (mean gnmds) are from ordination with epsilon 0.9 and k=3; columns 45 and 46 (mean gnmds) are from ordination with epsilon 0.95 and k=4
