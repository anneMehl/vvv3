...
for(i in 1:100)
{mds[[i]]<-monoMDS(dist.y, matrix(c(runif(dim(species)[1]*k)),nrow=dim(species)[1]), 
...
nmds2 <- metaMDS(species_sub10_1, k = 7, trymax = 100, trace = F, autotransform = FALSE, distance="bray")
plot(nmds2)
plot(nmds2, display = "sites", type = "n", ylim =  c(-3, 3), xlim = c(-3, 3))
...
# Legger på vektorer for environmental variables
  ...
  plot(mds.z, col = 1,  cex = 0.7, p.max = 0.05)
