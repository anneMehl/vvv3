...
## Make dataset with all explanatory variables of env.var_sub5 to use for the models
```{r}
...
#### Change levels for method, so that reference is in intercept
```{r}
levels(env.var_full_sub5$method)
env.var_full_sub5$method <- factor(env.var_full_sub5$method, levels = c("ref", "nat",  "pn",  "seed"))
plot(env.var_full_sub5$method)
