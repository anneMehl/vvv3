...
#### Summarise over rows and columns
```{r}
...
# data_species_wide_mean_sd <- data_species_wide %>%
#   summarise(
#     across(anemone_nemorosa:lolium_perenne, mean_sd),
#      
#   )
...
data_species_wide_temp <- bind_rows(data_species_wide_temp, data_species_wide_sd)
...
data_species_wide2 <- data_species_wide2 %>% rowwise() %>% mutate(m = mean(c_across(taraxacum_sp:lolium_perenne))) %>%
...
save(data_species_wide2, file = "data_species_wide2_2020_06_26.rda") # Includes mean and sd of species for columns and rows
